...
- 如果TC协调器检测到服务B执行本地jdbc提交的时候，失败，则要么让TC协调器代理B继续执行，要么一直通信告诉B继续执行（后者技术实现上，方便一些。TC里面不会有jdbc操作的代码，TC仅做协调作用），直到成功。需要保证事务的幂等性，可以用全局事务id保证。总结就是：commit指令告诉了TC，则无论什么情况，mysql宕机重启，还是微服务重启，都必须依靠TC的高可用集群，保证事务最终落盘。。其中TC可以用raft协议保证高可用性。
...
# 技术实现方案
- 就像上面那段伪代码一样，也像sharding-jdbc一样实现jdbc底层接口，接管sql的所有操作即可。可以做到对项目代码的0侵入性，现有项目的代码几乎0改动
